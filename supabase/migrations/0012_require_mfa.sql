-- Dashboard access requires both the server-managed admin role and a
-- successfully verified second factor in the current session.

drop policy if exists events_admin_all on public.events;
create policy events_admin_all on public.events
  for all to authenticated
  using (
    (select auth.jwt() -> 'app_metadata' ->> 'pwrhaus_role') = 'admin'
